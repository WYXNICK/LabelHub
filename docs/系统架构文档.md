...
队列库和对象存储属于后续阶段确认项；一旦确认，必须先更新 SDD 与本文档再进入实现。LLM 调用统一走 OpenAI API 兼容 HTTP 契约，不绑定供应商私有 SDK；题目级 LLM 辅助和阶段 4 AI 预审 Agent 已复用相同配置读取、超时控制和结构化输出校验。真实 API Key 不进入仓库；默认保持标准 Chat Completions 请求，如供应商需要额外请求体参数，统一通过 `OPENAI_EXTRA_BODY_JSON` / `LLM_EXTRA_BODY_JSON` 显式配置。
...
- 已创建 React/Vite 前端工程、FastAPI 后端工程和 Python Agent worker。
- 已实现登录、退出、当前用户、健康检查、OpenAPI、统一错误结构和 users 表迁移骨架。
- 已实现 Owner、Labeler、Reviewer 三类角色入口和受保护应用壳。
- 已准备 MySQL/Redis 本地 Docker Compose；任务、数据集、导入、审核配置、发布检查、状态迁移、审计日志、模板草稿、模板版本、assignment、submission、题目级 LLM 调用记录、AI 预审 job 和人工审核记录已接入 MySQL。
- 阶段 1 已完成 Owner 前置准备链路；阶段 2 已完成动态模板 Designer/Renderer、官方要求物料、进阶布局/规则、模板版本发布和发布检查联动；阶段 3 已完成 Labeler 任务广场、领取、作答、草稿、提交、我的贡献、返修入口和题目级 LLM 辅助。
- 阶段 4 已完成提交后 AI 自动预审、Agent 领取与写回、失败重试和人工兜底、AI 预审队列、人工审核任务列表、任务内复审/终审工作台、单条/批量通过或打回、直接修订入库、关键审计时间线、审核结果追溯和 Owner 数据验收统计。
- 阶段 5.0-5.3 已实现导出任务数据底座、Owner 导出中心、字段映射配置、导出任务创建、导出历史、JSON/JSONL/CSV/Excel 文件生成、真实下载和导出审计；阶段 5.4-5.5 继续打磨失败处理、下载审计展示和交付材料。
- 当前不配置 CI/CD。
