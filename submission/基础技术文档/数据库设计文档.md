...
erDiagram
  ...
  assignments ||--o{ submissions : creates
  submissions ||--o{ review_jobs : reviewed_by_ai
  submissions ||--o{ reviews : reviewed_by_human
  tasks ||--o{ export_jobs : exports
  users ||--o{ audit_logs : acts
```

当前代码已按该逻辑模型完成 MySQL Entity、Alembic 迁移和业务接口接入，覆盖用户、任务、数据集、导入、审核配置、模板草稿、模板版本、标注领取、草稿、提交、题目级 LLM 辅助、AI 预审、人工审核、审计日志、导出任务和导出文件元数据。
