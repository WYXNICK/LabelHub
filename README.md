...
当前仓库已经初始化为 monorepo。阶段 0 工程底座、阶段 1 Owner 任务/数据/审核配置/发布前检查底座，以及阶段 2.0-2.1 动态模板契约、数据库底座和 schema 校验已落地；任务创建、数据导入、题目预览、批量编辑、审核配置版本、发布阻塞检查和模板草稿保存已经接入 MySQL。模板 Designer/Renderer、Labeler 作答、AI 预审、人工审核和导出仍属于后续阶段。
...
当前 `GET /api/health`、`POST /api/auth/login`、`GET /api/auth/me`、`POST /api/auth/logout` 仍使用内存 demo 用户和 Cookie Session；除此之外，阶段 1 的任务创建、数据导入、题目批量编辑、审核配置版本、发布前检查依赖 MySQL 数据。阶段 2.0 已把 `template_drafts`、`template_versions` 表纳入 MySQL 迁移；阶段 2.1 已接入模板草稿保存和 schema 校验；模板 Renderer、Designer 和模板版本发布会在 2.2-2.7 分粒度接入业务逻辑。
