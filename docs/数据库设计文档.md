...
| `status` | varchar | `PENDING_HUMAN`、`APPROVED`、`RETURNED` |
...
| `human_conclusion` | varchar | `APPROVE`、`RETURN`，人工终审结果 |
...
当前设计为覆盖官方全流程的逻辑模型。实际落库按阶段推进：阶段 0 已落 `users`；阶段 1 已落任务、数据集、导入、审核配置版本、发布检查依赖字段和审计；阶段 2.0-2.7 已落模板草稿、模板版本、版本发布和任务发布检查联动；阶段 3 已落标注领取、草稿、提交版本和题目级 LLM 辅助记录；阶段 4 再落 `review_jobs`、AI 预审结果、人工审核记录与多轮审核审计；阶段 5 再落导出任务、导出文件和导出字段映射。
