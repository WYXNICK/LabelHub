...
队列库和对象存储属于后续阶段确认项；一旦确认，必须先更新 SDD 与本文档再进入实现。LLM 调用统一走 OpenAI API 兼容 HTTP 契约，不绑定供应商私有 SDK；题目级 LLM 辅助已复用该契约，阶段 4 的 AI 预审 Agent 也应复用相同配置读取和结构化输出校验。真实 API Key 不进入仓库；关闭 thinking 时必须随 Chat Completions 请求携带 `chat_template_kwargs.enable_thinking=false` 或等价 Provider 扩展参数。
