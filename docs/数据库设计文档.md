...
当前代码状态：阶段 0 已实现 `users` 表 Entity 与 Alembic 迁移；阶段 1 已将任务、数据集、导入、审核配置、文件对象、状态迁移和审计底座对应的 Entity、迁移与业务接口纳入代码。阶段 2-5 的模板、标注、审核、导出等后续实体仍需在正式实现前同步更新前后端 SDD、Entity 和迁移脚本。
...
当前设计为覆盖官方全流程的逻辑模型。实际落库按阶段推进：阶段 0 已落 `users`；阶段 1 已落任务、数据集、导入、审核配置版本、发布检查依赖字段和审计；阶段 2 起再落模板、标注、审核和导出相关实体。
