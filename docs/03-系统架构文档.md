...
LabelHub/
  apps/
    ...
    api/                  # 后端 API 服务，负责鉴权、业务状态、数据访问
    agent/                # AI 审核 Agent，消费队列并调用 LLM
  packages/
    shared/               # 共享类型、枚举、状态机定义、错误模型
    schema/               # 模板 schema、DTO schema、LLM 输出 schema
    ui/                   # 可复用 UI 组件和领域展示组件
    config/               # tsconfig、eslint、prettier 等共享配置
  infra/
    docker/               # 本地开发依赖，例如 MySQL、Redis
    migrations/           # 数据库迁移或迁移说明
  docs/                   # 项目文档
  demo_data/              # 课题提供的 demo 数据
  submission/             # 答辩提交材料，后续阶段创建
```
...
- `packages/shared` 和 `packages/schema` 是前后端契约核心。
...
- 服务端数据使用 TanStack Query 或同类方案。
- 页面局部状态使用 Zustand 或 React state。
- 表单状态使用 Formily 或同类动态表单内核。
...
AI Agent 建议作为独立 worker 运行，属于后端系统的一部分，但与 API 服务进程分离。
...
课题允许后端技术栈自选。考虑 monorepo、类型共享和动态 schema，建议：

| 层 | 建议 |
| --- | --- |
| 前端 | React 18 + TypeScript + Vite |
| UI | Ant Design 或 Semi Design，二选一保持统一 |
| 动态表单 | Formily 或自研 schema adapter |
| 拖拽 | dnd-kit |
...
| LLM 接入 | OpenAI-compatible adapter |
