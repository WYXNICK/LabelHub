...
LabelHub/
  ...
  infra/
    ...
    migrations/           # 数据库迁移或 Alembic 迁移说明
  docs/                   # 项目文档
...
- `packages/schema` 是前后端契约核心；后端通过 Pydantic/OpenAPI 暴露契约，前端按契约生成或维护 TypeScript 类型。
...
队列库、对象存储和具体 LLM SDK/client 属于后续阶段确认项；一旦确认，必须先更新 SDD 与本文档再进入实现。
