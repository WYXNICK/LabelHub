...
当前代码状态：阶段 0 已实现 `users` 表 Entity 与 Alembic 迁移；阶段 1.0 已将任务、数据集、导入、审核配置、文件对象、状态迁移和审计底座对应的 Entity 与 Alembic 迁移纳入代码。阶段 2-5 的模板、标注、审核、导出等后续实体仍需在正式实现前同步更新前后端 SDD、Entity 和迁移脚本。
