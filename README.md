...
当前仓库已经初始化为 monorepo。阶段 0 工程底座、阶段 1 Owner 任务/数据/审核配置/发布前检查底座，以及阶段 2.0-2.7 动态模板契约、数据库底座、schema 校验、Renderer、Designer、基础/高级物料、高级布局和规则、模板版本发布与发布检查联动已落地；任务创建、数据导入、题目预览、批量编辑、审核配置版本、发布阻塞检查、模板草稿保存和模板版本发布已经接入 MySQL。Labeler 作答、AI 预审、人工审核和导出仍属于后续阶段。
...
当前 `GET /api/health`、`POST /api/auth/login`、`GET /api/auth/me`、`POST /api/auth/logout` 仍使用内存 demo 用户和 Cookie Session；除此之外，阶段 1 的任务创建、数据导入、题目批量编辑、审核配置版本、发布前检查依赖 MySQL 数据。阶段 2.0 已把 `template_drafts`、`template_versions` 表纳入 MySQL 迁移；阶段 2.1-2.7 已接入模板草稿保存、schema 校验、Renderer、Designer、基础/高级物料、高级布局和规则校验、模板版本发布。模板版本发布成功后会更新 `tasks.current_template_version_id`，发布检查不再返回 `MISSING_TEMPLATE_VERSION`，但缺少数据集或审核配置时仍会继续阻塞发布。
