...
当前代码状态：阶段 0 已实现 `users` 表 Entity 与 Alembic 迁移；阶段 1 已将任务、数据集、导入、审核配置、文件对象、状态迁移和审计底座对应的 Entity、迁移与业务接口纳入代码；阶段 2.0 已将 `template_drafts`、`template_versions` 的 Entity、OpenAPI 契约和 Alembic 迁移纳入代码。阶段 2.1 起继续实现模板保存、校验、Renderer、Designer 和版本发布业务；阶段 3-5 的标注、审核、导出实体仍需在正式实现前同步更新前后端 SDD、Entity 和迁移脚本。
...
当前设计为覆盖官方全流程的逻辑模型。实际落库按阶段推进：阶段 0 已落 `users`；阶段 1 已落任务、数据集、导入、审核配置版本、发布检查依赖字段和审计；阶段 2.0 已落模板草稿与模板版本表；阶段 2.1 起再补模板业务校验，阶段 3-5 再落标注、审核和导出相关实体。
