...
- 阶段 5.2/5.3 创建导出任务后后端会同步生成 JSON、JSONL、CSV 或 Excel 文件；历史列表展示完成时间、文件名、文件大小和下载入口，失败时展示失败原因。
- 下载入口只在 `status=SUCCEEDED` 且 `fileObjectId` 存在时可用，点击进入 `GET /api/export-jobs/:exportJobId/download` 真实文件响应。
