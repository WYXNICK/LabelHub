...
| 后端 | Python |
| 数据库 | MySQL |
| LLM 接入 | OpenAI API 格式；Agent 默认 `BASE_URL=https://maas-coding-api.cn-huabei-1.xf-yun.com/v2`、`MODEL_NAME=astron-code-latest` |
...
队列库和对象存储属于后续阶段确认项；一旦确认，必须先更新 SDD 与本文档再进入实现。LLM 调用统一走 OpenAI API 兼容 HTTP 契约，不绑定供应商私有 SDK；题目级 LLM 辅助已复用该契约，阶段 4 的 AI 预审 Agent 也应复用相同配置读取和结构化输出校验。真实 API Key 不进入仓库；默认保持标准 Chat Completions 请求，如供应商需要额外请求体参数，统一通过 `OPENAI_EXTRA_BODY_JSON` / `LLM_EXTRA_BODY_JSON` 显式配置。
...
- 已准备 MySQL/Redis 本地 Docker Compose；任务、数据集、导入、审核配置、发布检查、状态迁移、审计日志、模板草稿、模板版本、assignment、submission 和题目级 LLM 调用记录已接入 MySQL。
...
- 当前已接入题目级 LLM 辅助，但尚未消费 AI 预审队列，不实现 Reviewer 流转和导出。
- 当前不配置 CI/CD。
