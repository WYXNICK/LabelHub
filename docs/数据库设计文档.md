...
- `field_mapping` 必须保存字段顺序、导出字段名、来源路径和是否包含审核字段，保证同一导出任务可以复现。
...
当前设计为覆盖官方全流程的逻辑模型。实际落库按阶段推进：阶段 0 已落 `users`；阶段 1 已落任务、数据集、导入、审核配置版本、发布检查依赖字段和审计；阶段 2.0-2.7 已落模板草稿、模板版本、版本发布和任务发布检查联动；阶段 3 已落标注领取、草稿、提交版本和题目级 LLM 辅助记录；阶段 4 已落 `review_jobs`、AI 预审结果、人工审核记录、直接修订、批量审核与多轮审核审计；阶段 5 再落导出任务、导出文件和导出字段映射。
