...
当前鉴权接口仍使用内存 demo 用户；任务、数据集、导入、审核配置、发布检查、状态迁移和审计日志已进入阶段 1 MySQL 主链路。阶段 2.0 已新增模板草稿与模板版本表、OpenAPI 契约和路由占位；阶段 2.1 已实现模板草稿保存和 schema 校验；模板 Renderer、Designer 和版本发布将在 2.2-2.7 继续实现。
