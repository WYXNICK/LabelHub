...
当前设计为覆盖官方全流程的逻辑模型。实际落库按阶段推进：阶段 0 已落 `users`；阶段 1 已落任务、数据集、导入、审核配置版本、发布检查依赖字段和审计；阶段 2.0-2.7 已落模板草稿、模板版本、版本发布和任务发布检查联动；阶段 3 已落标注领取、草稿、提交版本和题目级 LLM 辅助记录；阶段 4 已落 `review_jobs`、AI 预审结果、人工审核记录、直接修订、批量审核与多轮审核审计；阶段 5.0-5.1 已落 `export_jobs`、导出字段映射快照和导出任务创建审计，导出文件元数据会在文件生成粒度写入 `file_objects`。
