...
当前代码状态：阶段 0 已实现 `users` 表 Entity 与 Alembic 迁移；阶段 1 已将任务、数据集、导入、审核配置、文件对象、状态迁移和审计底座对应的 Entity、迁移与业务接口纳入代码；阶段 2.0-2.7 已完成 `template_drafts`、`template_versions` 的 Entity、OpenAPI 契约、Alembic 迁移、模板草稿保存、schema 校验、模板版本发布和发布检查联动；阶段 3 已完成 `assignments`、`submissions`、`llm_action_runs` 的 Entity、迁移、领取/草稿/提交/贡献/题目级 LLM 辅助接口；阶段 4 已完成 `review_jobs`、`reviews` 的 Entity、迁移、AI 预审写回、人工通过/打回/直接修订、批量审核、关键流转审计和 Owner 验收统计；阶段 5.0-5.3 已补齐 `export_jobs` Entity、迁移、字段映射快照、导出字段选项、导出任务创建/历史接口、导出文件生成和 `file_objects.purpose=EXPORT` 元数据关联。
...
- 如果后续开启一题多人标注，再调整为 `unique(task_id, dataset_item_id, labeler_id, round_no)` 或等价副本维度。
...
- 是否允许多人标注可后续扩展，届时需要引入分配轮次或标注副本维度。
...
- 阶段 5 导出只读取 `reviews.status=APPROVED` 且对应 `submissions.status=APPROVED` 的最终数据。
- `field_mappings` 必须保存字段顺序、导出字段名、来源路径和是否包含审核字段，保证同一导出任务可以复现。
- 下载文件通过 `file_objects` 关联，不把二进制内容写入业务表。
...
当前设计为覆盖官方全流程的逻辑模型。实际落库按阶段推进：阶段 0 已落 `users`；阶段 1 已落任务、数据集、导入、审核配置版本、发布检查依赖字段和审计；阶段 2.0-2.7 已落模板草稿、模板版本、版本发布和任务发布检查联动；阶段 3 已落标注领取、草稿、提交版本和题目级 LLM 辅助记录；阶段 4 已落 `review_jobs`、AI 预审结果、人工审核记录、直接修订、批量审核与多轮审核审计；阶段 5.0-5.3 已落 `export_jobs`、导出字段映射快照、导出任务创建/完成/失败/下载审计和 `file_objects.purpose=EXPORT` 导出文件元数据。
